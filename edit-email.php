            ...
            <ul class="nav nav-pills nav-stacked">
                ...
                while ($email = mysql_fetch_array($result)) {
                    if ($email["ID"] == (int)$_GET["ID"]) {
                        ?>
                        <li class="active"><a
                                href="edit-email.php?ID=<?= $email["ID"] ?>"><?= $email["Email_Name"] ?></a></li>
                    <?php
                    } else {
                        ?>
                ...
                while ($row = mysql_fetch_array($result)) {
                    ...
                    <form class="form-horizontal" action="doEditEmail.php" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input class="form-control disabled" type="text" disabled="disabled" name="Email_Name"
                                       id="Email_Name" value="<?php echo $row['Email_Name']; ?>"/>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input class="form-control" type="text" name="Email_Subject" id="Email_Subject"
                                       value="<?php echo $row['Email_Subject']; ?>"/>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control" name="Email_Body" rows="10"
                                          id="Email_Body"><?php echo $row['Email_Body']; ?></textarea>

                                <div class="help-block"><span style="cursor: pointer;" rel="popover"
                                                              title="Allowed Variable Tags" data-placement="bottom"
                                                              data-html="true"
                                                              data-content="{{usher name}}<br />{{usher email}}<br />{{event name}}<br />{{event time}}<br />{{event place}}<br />{{event date}}<br />{{event uc}}<br />{{Points}}<br />{{special instr}}<br />{{event uc name}}<br />{{meeting loc}}"
                                                              class="label label-info"><i class="fa fa-info-circle"></i> Allowed Tags</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input type="submit" class="btn btn-success btn-block" name="submit-email"
                                       id="submit-email" value="Edit Email"/>
                            </div>
                        </div>
